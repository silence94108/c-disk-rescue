...
- **安装版** `CDiskRescue_x.y.z_x64-setup.exe` — 双击安装,带开始菜单快捷方式和卸载项;缺 WebView2 时会自动引导安装。
- **便捷版** `CDiskRescue_x.y.z_x64-portable.exe` — 免安装,拷到哪双击就用、用完删掉不留痕。需系统自带 WebView2(Win11 及绝大多数 Win10 都有)。

系统要求:**Windows 10 1809+ / Windows 11,x64**。

**首次运行三步走**(暂未做数字签名,原因见下方「安全与隐私」):

1. **解除网络锁**——最常见的"双击没反应"就是它:右键下载的 exe → 属性 → 勾选底部「解除锁定」→ 确定;或 PowerShell 一行 `Unblock-File .\CDiskRescue_*.exe`。**每下载一次新版本都要解一次**,解完那个文件就一直能开。
2. 若弹「Windows 已保护你的电脑」→ 点「更多信息」→「仍要运行」。
3. 若被 360 / 腾讯电脑管家等拦截 → 在其信任区/白名单放行即可。
